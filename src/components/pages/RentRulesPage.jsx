  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6 space-y-5">
      ...
      <div className="text-center">
        <div className="inline-flex items-center gap-2 bg-gradient-to-r from-orange-500 to-amber-500 text-white px-5 py-2 rounded-full text-base font-bold shadow-md shadow-orange-200">
          🎲 租借遊戲規章
        </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h2 className="font-bold text-amber-800 text-sm">押金規定</h2>
        </div>
      ...
      <div className="bg-white border border-stone-200 rounded-2xl overflow-hidden shadow-sm">
        <div className="bg-gradient-to-r from-orange-500 to-amber-500 px-4 py-2.5 flex items-center gap-2">
          ...
          <h2 className="font-bold text-white text-sm">租金計算表</h2>
        </div>
        <div className="divide-y divide-stone-100">
          {PRICE_TIERS.map((tier, i) => (
            <div key={i} className="flex items-center justify-between px-4 py-3">
              <span className="text-stone-600 text-sm">{tier.range}</span>
              <span className={`font-bold text-sm ${typeof tier.rent === 'number' ? 'text-orange-600' : 'text-stone-400'}`}>
      ...
      <div className="bg-white border border-stone-200 rounded-2xl overflow-hidden shadow-sm">
        <div className="bg-stone-700 px-4 py-2.5 flex items-center gap-2">
          <span className="text-white text-sm">📋</span>
          <h2 className="font-bold text-white text-sm">注意事項</h2>
        </div>
        <ul className="divide-y divide-stone-100">
          {RULES.map((rule, i) => (
            <li key={i} className="flex items-start gap-3 px-4 py-3">
              <span className="mt-0.5 shrink-0 w-5 h-5 rounded-full bg-orange-100 text-orange-600 text-xs font-bold flex items-center justify-center">
                {i + 1}
              </span>
              <span className="text-stone-600 text-sm leading-relaxed">{rule}</span>
            </li>
          ))}
        </ul>
      </div>
