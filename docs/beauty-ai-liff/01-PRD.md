...
| 編號 | 需求 | 優先 |
|---|---|---|
| FR-CTA-1 | 「為你挑的保養 / 彩妝」：依膚質與季型從商品對照表挑 3 件，點擊帶 UTM 到官網 | P0 |
| FR-CTA-2 | 「預約彩妝課 / 一對一諮詢」：開啟 LINE 預約（或 Google 表單 / 現有預約系統），預填臉型與季型 | P0 |
...
| FR-TAG-2 | 點擊 CTA 後追加 `intent:product` / `intent:class` / `intent:share` | P0 |
...
| **8 行動** | 點商品 / 預約 / 分享 | 三個 CTA | 「這支唇膏是我的色」🛍️ | 硬推銷感 | CTA 文案用「為你挑的」；商品只列 3 件 | `cta_click:{type}` |
...
1. 彩妝課預約走現有預約系統、LINE 官方預約、還是 Google 表單？
2. 商品對照表由誰維護？建議先用 Google Sheet（Han 已有 Sheet 工作流），後端每小時快取。
3. 分享卡是否允許放客人照片？（預設不放）
