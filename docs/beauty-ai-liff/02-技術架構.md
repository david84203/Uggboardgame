...
| 型錄沒有「膚質 / 季型 → 商品」的規則 | 型錄是全品項清單，對照表是 Han 的專業判斷 | Han 從型錄挑出 15–30 件常推品項，填進 Sheet 的 `skin_types`、`observations`、`seasons` 欄 |
...
**選品規則**（後端，Han 決策：保養與彩妝各半）：先過濾 `active`；保養類依膚質相符過濾、再依 `observations` 命中數加分，取前 2 件；彩妝類依 `seasons` 相符過濾、再依膚質（底妝）加分，取前 2 件；同分依 `priority`。共 **4 件**，任一類不足 2 件時以「全膚質 / 全季型」的通用品項補位。若你已有現成的商品清單或別人的對照表，只要能整理成上面的欄位就能直接用；**品項與規則本身建議由 Han 自己的產品線定義**，因為導購連結要導回自家官網。
