...
| Rich Menu | 2500×1686、六格版型（Han 現有：淨透養膚 / 澎潤Q彈 / 體態輕盈 / 韓式照相館 / 認識Han / 逛逛更多）；建議以「AI 妝容診斷」取代「逛逛更多」並移到左上（見 06 文件第 2 節）；該格 action = `uri`，值為 `https://liff.line.me/{LIFF_ID}?ref=menu` |
...
├── cta/ProductPicks.jsx    # 依膚質 / 色調挑 4 件
...
// Response 200
{ "analysisId": "abc123", "result": { /* analysis-schema.json */ }, "products": [ /* 4 件：2 保養 + 2 彩妝 */ ] }
...
6. 從商品對照表（Google Sheet 匯出的 JSON，function 內快取 1 小時）挑 4 件（2 保養 + 2 彩妝），回傳。
...
- `{OA_ID}` 是官方帳號的基本 ID，含 `@`，要 URL-encode 成 `%40`。Han 的帳號為 `@726rmfol`，完整範例：`https://line.me/R/oaMessage/%40726rmfol/?%E6%88%91%E6%83%B3%E9%A0%90%E7%B4%84%E5%BD%A9%E5%A6%9D%E8%AA%B2`。
...
後端同時在 `users/{uid}.tags` 追加 `intent:class`。「我想預約拍照」與「我想了解這 4 件」用同一機制，分別追加 `intent:photo`、`intent:product`。Han 在聊天室看到「報告編號」後，可到店員 APP 的報告查詢頁（M8）用編號打開完整報告。

### 4.6 商品對照表：是什麼、怎麼用

**用途**：分析完成後，「為你挑的保養 / 彩妝」區塊要從你的商品裡挑 4 件（2 保養 + 2 彩妝）。挑選依據是 AI 回傳的 `skin.type`（膚質）、`skin.observations`（泛紅、毛孔…）、`color_tone.tone`（冷暖色調），臉型只影響妝容畫法、不影響選品。對照表就是「哪種膚質 / 季型 → 推哪些商品」的規則表，由 Han 維護。

**格式**：一張 Google Sheet，每列一件商品，後端每小時抓成 JSON 快取。

| 欄位 | 說明 | 範例 |
|---|---|---|
| `sku` | 系統內商品代碼（唯一） | `SERUM-01` |
| `mk_code` | 玫琳凱型錄貨號（給 Han 下單對照用，不顯示給客人） | `10-123456` |
| `name` | 顯示名稱 | 積雪草舒緩精華 |
| `category` | `skincare` / `makeup` | `skincare` |
| `url` | 客人打得開的商品頁（Han 的顧問個人網站）；**可留空**，留空則該商品只走「詢問清單」 | `https://www.marykay.com.tw/...` |
| `image` | 商品圖網址（正方形） | … |
| `price` | 顯示價格 | `880` |
| `skin_types` | 適用膚質，逗號分隔；空白 = 全部 | `oily,combination` |
| `observations` | 命中這些觀察就加分，逗號分隔 | `redness,acne` |
| `tones` | 適用色調（彩妝用）：`warm` / `cool` / `neutral`，逗號分隔；空白 = 全部 | `cool` |
| `priority` | 同分時的排序（數字小優先） | `1` |
| `active` | `TRUE` / `FALSE` | `TRUE` |

**商品來源：玫琳凱型錄（MK 管理後台）**

型錄可以當「商品來源」，但不能直接當對照表，也不能直接拿它的連結給客人，原因與做法如下：

| 問題 | 說明 | 做法 |
|---|---|---|
| 後台連結需要顧問登入 | MK 管理裡的型錄是顧問端頁面，客人點了會被要求登入或看到空白 | 對照表的 `url` 欄改填**客人打得開的**頁面：Han 的玫琳凱個人顧問網站商品頁（若有），或不填網址改走下方「詢問清單」流程 |
| 型錄沒有「膚質 / 色調 → 商品」的規則 | 型錄是全品項清單，對照表是 Han 的專業判斷 | Han 從型錄挑出 15–30 件常推品項，填進 Sheet 的 `skin_types`、`observations`、`tones` 欄 |
| 品牌素材使用規範 | 玫琳凱對獨立美容顧問的網路宣傳與商標、圖片使用有規範 | 商品圖與文案用公司提供給顧問的素材；上線前依顧問手冊確認一次即可，不影響架構 |
| 玫琳凱是顧問直銷模式 | 客人最終多半是跟顧問下單，而不是在公開網站結帳 | **建議商品 CTA 預設走「加入詢問清單」**：點「我想了解這 3 件」→ 用與預約相同的 oaMessage 機制開 Han 的 LINE 對話框，預填「我對 A、B、C 有興趣＋精簡報告」。這最貼合玫琳凱的成交方式，也不需要任何外部商城 |

對照表因此新增一欄 `mk_code`（玫琳凱型錄貨號），方便 Han 在 LINE 收到詢問時直接對照下單。範本見 [products-template.csv](./products-template.csv)，可直接匯入 Google Sheet 後填寫。

**選品規則**（後端，Han 決策：保養與彩妝各半）：先過濾 `active`；保養類依膚質相符過濾、再依 `observations` 命中數加分，取前 2 件；彩妝類依 `tones` 相符過濾、再依膚質（底妝）加分，取前 2 件；同分依 `priority`。共 **4 件**，任一類不足 2 件時以「全膚質 / 全色調」的通用品項補位。若你已有現成的商品清單或別人的對照表，只要能整理成上面的欄位就能直接用；**品項與規則本身建議由 Han 自己的產品線定義**，因為導購連結要導回自家官網。

### 4.4 Vercel Cron `0 18 * * *`（台灣 02:00）
把前一日新增 / 變更標籤的使用者同步進 LINE 受眾群組（`POST /v2/bot/audienceGroup/upload` 建群，`PUT /v2/bot/audienceGroup/upload` 追加 userId），每個色調與膚質各一群，供 OA 後台「分眾推播」直接選用。

## 5. AI 呼叫設計

> **2026-09-15 更新（Han 決策：系統化版）**：AI 只回傳五個分類（品質、臉型、量感、冷暖色調、膚質）與兩句話；臉部幾何與三庭五眼由 MediaPipe 在客端量測，腮紅 / 修容 / 打亮位置、色票、商品全由程式規則決定。實作細節見 07 文件。本節 5.2 的程式骨架已由 `api/beauty/_lib/ai/claude.js` 取代。
...
| 系統提示 | 放判斷準則（臉型量測定義、冷暖色調與明度彩度判斷法、膚質觀察項、語氣規範、圖庫代碼定義），約 2–3k tokens，加 `cache_control` 快取 |
...
users/{lineUserId}
  ...
  tags: ["face:oval","tone:cool","volume:light","skin:combination","style:korean_bare","stage:analyzed","intent:product"]
  daily: { "2026-09-15": 2 }
  consentAt, deletedAt?
...
config/products          // 由 Google Sheet 同步的商品對照表（skinType × tone → sku[]）
...
- **金鑰**：`ANTHROPIC_API_KEY`、`LINE_CHANNEL_ACCESS_TOKEN`、`LINE_LOGIN_CHANNEL_ID`、Firebase service account 全在 Vercel 環境變數；前端只暴露 `VITE_LIFF_ID` 與 `VITE_HAN_OA_ID`（Han 官方 LINE 的 `@` ID：**`@726rmfol`**，URL 中寫成 `%40726rmfol`；未設定時預約與詢問按鈕自動隱藏並記錄警告）。
