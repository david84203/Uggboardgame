...
- **金鑰**：`ANTHROPIC_API_KEY`、`LINE_CHANNEL_ACCESS_TOKEN`、`LINE_LOGIN_CHANNEL_ID`、Firebase service account 全在 Vercel 環境變數；前端只暴露 `VITE_LIFF_ID` 與 `VITE_HAN_OA_ID`（Han 官方 LINE 的 `@` ID，**目前留空**，上線前補上；未設定時預約與詢問按鈕自動隱藏並記錄警告）。
